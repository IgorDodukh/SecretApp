package AppMain.Selenium.Settings;

import AppMain.FXUI.ProgressBar;
import AppMain.Selenium.Tests.AddNewCustomer;
...
//    }
    @BeforeSuite
    public void setUp(int environmentComboBoxIndex, int browserComboBoxIndex, WebDriver driver) {
        this.driver = new ChromeDriver();

        System.out.println("Run WebDriver");
        setTotalResultMessage(getTotalResultMessage() + "Run WebDriver\n");
        ProgressBar.addProgressValue(progressVariable);
        this.driver.manage().window().setSize(new Dimension(1366, 900));
//        magentoFSLink = fsEnvironment.get(envIndex);
        this.driver.get(fsEnvironment.get(2));
    }
